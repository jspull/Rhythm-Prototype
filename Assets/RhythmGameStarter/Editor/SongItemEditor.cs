...
namespace RhythmGameStarter
{
    ...
    public class SongItemEditor : Editor
    {
        ...
        private AnimBool songVisibleAnim, sequenceVisibleAnim, metadataVisibleAnim, generatorVisibleAnim, customMappingVisibleAnim;
        ...
        private void OnEnable()
        {
            ...
            useGenetatorProperty = serializedObject.FindProperty("useGenerator");
            ...
            customMappingVisibleAnim = new AnimBool(Repaint);
            customMappingVisibleAnim.speed = 8f;
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (EditorGUILayout.BeginFadeGroup(songVisibleAnim.faded))
            {
                ...
                EditorGUILayout.PropertyField(serializedObject.FindProperty("coverArt"));
            ...
            if (EditorGUILayout.BeginFadeGroup(sequenceVisibleAnim.faded))
            {
                ...
                // EditorGUILayout.LabelField("Note names : " + songItem.GetNoteNames().Aggregate("",(a, b) => a + b + ", ").TrimEnd(',', ' '));
                 EditorGUI.EndDisabledGroup();

                // create an editor button to create an MidiTrackMapping scriptable object with undo support
                if (GUILayout.Button("Create MidiTrackMapping"))
                {
                    // get the assets path of the current songItem
                    var path = AssetDatabase.GetAssetPath(songItem);

                    // go up one directory of the path 
                    path = path.Substring(0, path.LastIndexOf(".asset"));
                    var midiTrackMapping = ScriptableObject.CreateInstance<MidiTrackMapping>();
                    // set the mappings to songItem.GetNoteNames()
                    midiTrackMapping.mapping = songItem.GetNoteNames().Select(x => new MidiTrackMapping.Track { noteTarget = x }).ToList();
                    midiTrackMapping.ignoreOctave = true;
                    AssetDatabase.CreateAsset(midiTrackMapping, path + "_MidiTrackMapping.asset");
                    AssetDatabase.SaveAssets();
                    Selection.activeObject = midiTrackMapping;
                }
                EditorUtils.EndIndent();
            }
            EditorGUILayout.EndFadeGroup();

            customMappingVisibleAnim.target = EditorUtils.Foldout(customMappingVisibleAnim.target, "Mapping Override");
            if (EditorGUILayout.BeginFadeGroup(customMappingVisibleAnim.faded))
            {
                EditorUtils.Indent();
                EditorGUILayout.PropertyField(serializedObject.FindProperty("customMidiMapping"));
                EditorGUILayout.PropertyField(serializedObject.FindProperty("customPrefabMapping"));
                EditorUtils.EndIndent();
            }
            EditorGUILayout.EndFadeGroup();
